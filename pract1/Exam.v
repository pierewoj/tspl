...
Inductive member {X : Type} : X -> list X -> Prop :=
   ...
   | member_later : forall (x:X) xs x y,
...
Proof.
  ...
  - simpl. apply member_later. assumption. assumption.
Qed.
...
Proof.
   ...
   - simpl in H. inversion H. subst.
     ...
     + subst. apply IHxs in H2. destruct H2. 
       * left. apply member_later. assumption. assumption.
       * right. assumption.
Qed.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_RepeatEnd : forall c st st' b,
      c / st || st' -> 
      beval st' b = true ->
      (REPEAT c UNTIL b END) / st || st'

  | E_RepeatLoop : forall c st st' st'' b,
      c / st || st' -> 
      beval st' b = false -> 
      (REPEAT c UNTIL b END) / st' || st'' ->
      (REPEAT c UNTIL b END) / st || st''
...
Proof.
  intros P Q b c HHoare HP st st' He.
  remember (REPEAT c UNTIL b END) as wcom eqn:Heqwcom.
  induction He;
    try (inversion Heqwcom); subst; clear Heqwcom.
    - split.  apply (HHoare st st'). assumption. assumption.
      apply bexp_eval_true. assumption.
    - intros. apply IHHe2. reflexivity.
      apply (HP st'). split.
