...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_RepeatEnd : forall st c1 st' b1,
      ceval st c1 st' ->
      beval st' b1 = true ->
      ceval st (REPEAT c1 UNTIL b1 END) st'
   | E_RepeatLoop : forall st c1 st' st'' b1,
      ceval st c1 st' ->
      beval st' b1 = false ->
      ceval st' (REPEAT c1 UNTIL b1 END) st'' ->
      ceval st (REPEAT c1 UNTIL b1 END) st''.
...
Theorem hoare_repeat : 
  forall P Q c b, 
  {{fun st => P}} c {{fun st => Q}} -> 
  {{fun st => Q /\ ~ (bassn b st)}} c {{fun st => Q}} -> 
  {{fun st => P}} REPEAT c UNTIL b END {{fun st => Q /\ (bassn b st)}}.
Proof.
  intros.
  intros st st' HRep.
   remember (REPEAT c UNTIL b END) as wcom eqn:Heqwcom.
  induction HRep; try (inversion Heqwcom).
  - split. apply (H st st'). rewrite <- H3. assumption. assumption.
    apply bexp_eval_true. rewrite <- H4. assumption.
  - split. apply (H st st'). rewrite <- H3. assumption. assumption.
    apply IHHRep2. rewrite H3. rewrite H4. reflexivity. assumption.
Qed.
