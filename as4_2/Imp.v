...
(**  It's a bit silly that we prove all these awkward versions
...
Inductive bevalR : bexp -> bool -> Prop :=
  ...
  | E_Beq : forall (e1 e2: aexp)  (n1 n2: nat) (br1: bool) ,
       aeval e1 = n1 ->
       aeval e2 = n2 ->
       beq_nat n1 n2 = br1 -> 
       bevalR (BEq e1 e2) (br1)
  | E_Ble : forall (e1 e2: aexp) (n1 n2: nat) (br1: bool) ,
       aeval e1 = n1 ->
       aeval e2 = n2 ->
        leb n1 n2 = br1 -> 
       bevalR (BLe e1 e2) (br1)
  | E_BNot : forall (b1 : bexp) (br1 : bool), 
       bevalR b1 (negb br1) -> bevalR (BNot b1) br1
  | E_BAnd : forall (b1 b2 : bexp) (br1 br2 br3: bool),
      bevalR b1 br1 -> bevalR b2 br2 -> 
      andb br1 br2 = br3 ->
      bevalR (BAnd b1 b2) br3
.
Theorem beval_iff_bevalR' : forall exp res,
  bevalR exp res <-> beval exp = res.
Proof.
  intros. split.
  - intros H. induction H; simpl; subst; try reflexivity. 
     rewrite IHbevalR. rewrite negb_involutive. reflexivity.
  - intros H. generalize dependent res. induction exp. 
      + intros. simpl in H. rewrite <- H. apply E_BTrue.
      + intros. simpl in H. rewrite <- H. apply E_BFalse.
      + intros. simpl in H. apply E_Beq with (n1 := aeval a) (n2 := aeval a0).
        reflexivity. reflexivity. apply H.
      + intros. apply E_Ble with (n1 := aeval a) (n2 := aeval a0).
        reflexivity. reflexivity. apply H.
      + intros. apply E_BNot. simpl. simpl in H. destruct res.
        * simpl. apply IHexp. apply negb_true_iff. assumption.
        * simpl. apply IHexp. apply negb_false_iff. assumption.    
      + intros. simpl in H. apply E_BAnd with (br1 := beval exp1) (br2 := beval exp2).
        * apply IHexp1. reflexivity.
        * apply IHexp2. reflexivity.
        * assumption.
Qed.
(** [] *)
...
Theorem no_whiles_terminating: 
  forall st c, no_whiles c = true -> exists st', c / st \\ st'.
Proof.
  ...
  - intros st. inversion H. apply andb_true_iff in H1. inversion H1. 
    destruct IHc1 with (st := st). apply H0.
    destruct IHc2 with (st := x). apply H2.
    exists x0.  apply E_Seq with (st' := x). assumption. assumption.
  - intros st. inversion H. apply andb_true_iff in H1. inversion H1.
    destruct IHc1 with (st := st). apply H0.
    destruct IHc2 with (st := st). apply H2.
    destruct (beval st b) eqn:HDestr.
    +  exists x. apply E_IfTrue. assumption. assumption.
    +  exists x0. apply E_IfFalse. assumption. assumption.
  - intros st. simpl in H. inversion H.
Qed.
