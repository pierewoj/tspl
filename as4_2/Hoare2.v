...
(* this was as a homework before*)
Theorem mult_S_r : forall m n, m * S n = m + (m * n).
Admitted.

(*factorial (st X) * st Y = factorial m*)
Theorem factorial_fact2:
  forall n:nat,
  factorial (n)*S(n) = factorial(S(n)).
Proof.
  intros n.
  simpl. rewrite mult_S_r. rewrite mult_comm. reflexivity.
Qed.

Theorem factorial_exercise : 
  forall m:nat,
  {{ fun st => st X = m }} 
  factorial_program
  {{ fun st => st Y = factorial m }}.
Proof.
  intros m.
  unfold factorial_program.
  apply hoare_seq with (Q:= fun st => (factorial (st X))*st Y = factorial m ).
  apply hoare_consequence_post with
   (Q' := fun st => (factorial (st X))*st Y = factorial m /\
    ~ bassn (BNot (BEq (AId X) (ANum 0))) st ).
  apply hoare_while.
  apply hoare_seq with (Q:= fun st : state => factorial ((st X) - 1) * (st Y)  = factorial m).
  - apply hoare_asgn with (Q := fun st => factorial (st X) * st Y = factorial m).
  - apply hoare_consequence_pre with 
(P' := (fun st : state => (factorial (st X - 1)) * st Y = factorial m)[Y |-> AMult (AId Y) (AId X) ]). 
     * apply hoare_asgn with (Q := (fun st : state => factorial (st X - 1) * st Y = factorial m)  ) .
     * simpl. unfold assert_implies. intros. unfold assn_sub. unfold t_update. simpl.  
        rewrite -> mult_comm with (n := st Y) (m := st X). 
        rewrite -> mult_assoc. inversion H. inversion H1. apply negb_true_iff in H3. apply beq_nat_false in H3.
        destruct (st X).
        contradiction H3. 
        reflexivity. simpl. rewrite <- minus_n_O. rewrite factorial_fact2. apply H0.
   - unfold assert_implies. intros. inversion H. unfold bassn in H1. simpl in H1. apply eq_true_negb_classical in H1.
      apply beq_nat_true in H1. rewrite H1 in H0. simpl in H0. rewrite <- H0. rewrite plus_0_r. reflexivity.
   - apply hoare_consequence_post with (Q' :=  fun st : state => st X = m /\ st Y = 1).
      *  apply hoare_consequence_pre with (P' := (fun st : state => st X = m /\ st Y = 1)[Y |-> ANum 1]).
          apply hoare_asgn with (Q := fun st : state => st X = m /\ st Y = 1).
          unfold assert_implies. intros. unfold assn_sub. unfold t_update. simpl. split. assumption. reflexivity.
      * unfold assert_implies. intros. inversion H. rewrite -> H0. rewrite -> H1. simpl. rewrite mult_1_r. reflexivity.
Qed.  
...
(
      {{ fun st => st X =  m }} ->>
    {{ (fun st => (real_fact (st X))*st Y = real_fact m) [Y |-> ANum 1] }}
  Y ::= ANum 1
...
Proof.
  ...
  - destruct (st X). 
    ...
    + rewrite <- mult_assoc. rewrite mult_comm with (m:= S n). rewrite mult_assoc.
      (* i proved factorial fact 2 in the previous part of this file *)
      simpl. rewrite <- minus_n_O. rewrite factorial_fact2. assumption.
  - rewrite mult_assoc. assumption. 
  - simpl in H. rewrite plus_0_r in H. assumption.
Qed. 
