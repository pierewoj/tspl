...
(* !!! here a lot of code was changed, added fix/let/empty *)
...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  (*tlet*)
  | tlet id t1 t2 
                    => tlet id (subst x s t1) (if beq_id x id then t2 else subst x s t2)
  | tinl T t1 =>
      tinl T (subst x s t1)
  | tinr T t1 =>
      tinr T (subst x s t1)
...
(*tfix*)
  | tfix tm => tfix (subst x s tm)

(*temptycase*)
  | temptycase tm => temptycase (subst x s tm)
  end.
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_SndPair : forall v1 v2,
        ...
        (tsnd (tpair v1 v2)) ==> v2
  (* let *)
  | ST_Let1 : forall id t1 t1' t2,
         t1 ==> t1' ->
         (tlet id t1 t2) ==> (tlet id t1' t2)
  | ST_LetValue : forall id v1 t2,
         value v1 ->
         (tlet id v1 t2) ==> subst id v1 t2
  (* sums *)
  ...
  | ST_FixAbs: forall xf T1 t2,
      tfix (tabs xf T1 t2) ==> subst xf (tfix (tabs xf T1 t2)) t2 
  (* empty *)
  | ST_Empty: forall t1 t1',
      t1 ==> t1' ->
      temptycase t1 ==> temptycase t1'
...
(* let *)
  - apply T_Let with T1...
    apply IHhas_type2. intros y Haf.
    unfold update, t_update.
    destruct (beq_idP x y)...
  - (* T_Case *)
    eapply T_Case...
...
(* let *)
  - rename i into y. apply T_Let with (T1 := T1).
    + eapply context_invariance...
    + destruct (beq_idP x y).
      * eapply context_invariance...
        subst. intros x Hafi. unfold update, t_update.
        destruct (beq_id y x)...
      * apply IHt2.
        eapply context_invariance...
        intros z Hafi.
