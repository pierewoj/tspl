...
(** We have seen that Coq has mechanisms both for _programming_,
    ...
    using inductive propositions (like [ev] or [eq]), implication,
    universal quantification, and the like.  So far, we have treated
    ...
    that Coq's programming and proving facilities are closely
    related. For example, the keyword [Inductive] is used to declare
    both data types and propositions, and [->] is used both to
    describe the type of functions on data and logical
    implication. This is not just a syntactic accident!  In fact,
    programs and proofs in Coq are almost the same thing.  In this
    chapter we will study how this works.
    ...
    evidence, which can be thought of as a data structure. If the
    proposition is an implication like [A -> B], then its proof will
    be an evidence _transformer_: a recipe for converting evidence for
...
(** The trick is to introduce an alternative pronunciation of "[:]".
    ...
    example, the second line in the definition of [ev] declares that
    [ev_0 : ev 0].  Instead of "[ev_0] has type [ev 0]," we can say
    that "[ev_0] is a proof of [ev 0]." *)
...
(** This can be read "[ev_SS] is a constructor that takes two
    arguments -- one number [n] and evidence for the proposition [ev
...
(** The expression [ev_SS 2 (ev_SS 0 ev_0)] can be thought of as
    ...
    specific argument [2] and [0] and the corresponding proof objects for
    its premises [ev 2] and [ev 0].  Alternatively, we can think of [ev_SS]
    as a primitive "evidence constructor" that, when applied to a
    particular number, wants to be further applied to evidence that
    that number is even; its type,
...
(** Notice that we're using [apply] here in a new way: instead of just
    ...
    type. *)
(**  AAA: This discussion is a little bit redundant given the
   previous explanation about using lemmas as functions. 
 *)

(* ##################################################### *)
(** * Proof Scripts and Proof Objects *)

(** These proof objects lie at the core of how Coq operates.  When Coq
    is following a proof script, what is happening internally is that
    it is gradually constructing a proof object -- a term whose type
    is the proposition being proved.  The tactics between the [Proof]
    command and the [Qed] instruct Coq how to build up a term of the
    required type.  To see this process in action, let's use the [Show
    Proof] command to display the current state of the proof tree at
...
(**  APT: Apparently no longer true in 8.4
    In the [Show Proof] output, lines of the form
    [?1 -> ev n] record these
...
(**  There used to be quizzes about beautiful here, we can make
   some about ev instead. 
 *)
(* ##################################################### *)
...
(** When we view the proposition being proved by [ev_plus4] as a
    ...
    argument, [n].  While such _dependent types_ are not commonly
    found in programming languages, even functional ones like ML or
...
(** Inductive definitions are powerful enough to express most
    connectives and quantifiers studied in the [Logic]
    chapter. Indeed, none of those is primitive in Coq: only universal
    quantification (and thus implication) is built-in; all the others
    are defined inductively. We study these definitions in this
    section. *)
...
(** ** Conjunction
    ...
    /\ Q] as consisting of a pair containing two proofs: one for [P]
    and another one for [Q]. This leads to the following
    definition. *)
...
(** Now it becomes clear why [destruct] and intro-patterns can be used
    on a conjunctive hypothesis. Case analysis allows us to consider
    all possible ways in which [P /\ Q] was proved, and only one case
    appears in the definition: The [conj] constructor. The [split]
    tactic is also not special, since it actually works for any
    inductively defined proposition with only one constructor. In
    particular, it works for our own definition of [and]: *)
...
(** This shows that the inductive definition can still be manipulated
    by tactics exactly the way we're used to. However, we can also use
    it to build proof directly through pattern-matching. For instance: *)
...
(** Similarly to the definition of conjunction, this declaration
    explains the behavior of [destruct] on a disjunctive hypothesis,
    since the generated subgoals match the shape of the [or_introl]
    and [or_intror] constructors. Once again, we can write proof
    objects for theorems involving [or] directly, without resorting to
    proof tactics. *)
...
(** ** Existential Quantification
    ...
    To define existential quantification, we package the witness of
    the existential [x] together with a proof that [x] satisfies the
    property [P] that we care about: *)
...
(** Note that the form [exists x, P x] is defined in the standard
    library, and desugars to an expression involving [ex]: *)

Check ex (fun n => ev n).
(* ===> exists n : nat, ev n
        : Prop *)

(** Here's how we may define an explicit proof object involving [ex]: *)

Definition some_nat_is_even : exists n, ev n :=
  ex_intro ev 4 (ev_SS 2 (ev_SS 0 ev_0)).

(** By matching on an existence proof, we get access to its witness: *)

Definition match_ex (n : nat) (H : exists p, ev (p + n))
                    : exists p, ev (2 + p + n) :=
  match H with
  | ex_intro p Hp => ex_intro (fun p => ev (2 + p + n))
                              p (ev_SS (p + n) Hp)
  end.
...
(** If we can build proofs with explicit terms rather than tactics,
    ...
    than explicit terms.  Of course the answer is yes! *)
...
(** Notice that we terminate the [Definition] with a [.] rather than
    ...
    like a normally-defined function.
...
(** Even Coq's equality relation is not built in.  It has (roughly)
    the following inductive definition (enclosed in a module to avoid
...
(** We can use [refl_equal] to construct evidence that, for example,
    [2 = 2].  Can we also use it to construct evidence that [1 + 1 =
    2]?  Yes: indeed, it is the very same piece of evidence!  The
...
(**  AAA: The following have been resurrected from the now dead
   MoreLogic.v (the end is marked below). It needs to be reintegrated here. 
 *)
(* ####################################################### *)
...
(**  AAA: The stuff brought back from MoreLogic ends here 
 *)
