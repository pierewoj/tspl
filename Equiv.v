...
Proof.
  intros.
  split.
  - intros. replace st' with (t_update st X (aeval st e)).
    + apply E_Ass. reflexivity.
    + apply functional_extensionality. intro.
      rewrite <- H. simpl. rewrite t_update_same. inversion H0.
      reflexivity.
  - intros. replace st' with st. apply E_Skip.
    inversion H0. subst. apply functional_extensionality.
    intro. rewrite <- H. simpl. rewrite t_update_same. reflexivity.
Qed. 
(** [] *)
...
Proof.
  intros. split. 
  - intros. remember (IFB b THEN c1 ELSE c2 FI) as cif eqn:Hif.
    induction H2; inversion Hif. (* we get rid of cases that could not happen *)
    + subst. apply E_IfTrue. rewrite H in H2. assumption. 
      apply H0. assumption.
    + subst. apply E_IfFalse. rewrite H in H2. assumption.
      apply H1. assumption.
   - intros. remember (IFB b' THEN c1' ELSE c2' FI) as cif eqn:Hif.
      induction H2; inversion Hif.
     + subst. apply E_IfTrue. rewrite H. assumption.
       apply H0. assumption.
